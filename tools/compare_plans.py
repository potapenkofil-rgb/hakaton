...
COLUMNS = ["plan_id", "scenario_id", "feasible", "hard_violations", "pv_total_mln", "total_cost_mln", "capex_total_mln",
...
def row(ctx, raw, scenario):
    ...
    t = r["totals"]
    return {
        "plan_id": raw["plan_id"], "scenario_id": scenario, "feasible": r["feasible"], "hard_violations": len(bad),
...
def main(argv=None):
    ...
    print(f"{'план':<{width}} {'сценарий':<16} {'статус':<9} {'PV':>8} {'всего':>8} {'CAPEX':>6} {'дефицит':>8} {'резерв':>7}  нарушения")
    for r in rows:
        print(f"{r['plan_id']:<{width}} {r['scenario_id']:<16} {'исполним' if r['feasible'] else 'НЕТ':<9} {r['pv_total_mln']:8.0f} {r['total_cost_mln']:8.0f} {r['capex_total_mln']:6.0f} {r['shortage_total_t']:8.1f} {r['min_reserve_days']:7.0f}  {r['violations']}")
